...
describe('FeaturesViewerFlowTest', function() {
    ...
    describe('Viewer initialization.', function() {
        ...
        it('should create 1 up_pftv_nav_buttons with 3 children', function() {
            var buttonsDiv = document.querySelectorAll('.up_pftv_container .up_pftv_nav_buttons table');
            assert.equal(buttonsDiv.length, 1, 'only one up_pftv_nav_buttons');
            assert.equal(buttonsDiv[0].childElementCount, 3, 'up_pftv_nav_buttons children count');

            var buttons = document.querySelectorAll('.up_pftv_nav_buttons span');
            assert.equal(buttons.length, 1, 'only one button');
            assert.equal(buttons[0].getAttribute('class'), 'fv-icon-zoom-in', 'zoom button class');

            var boxes = document.querySelectorAll('.up_pftv_nav_buttons input');
            assert.equal(boxes.length, 2, 'two text areas');
        });
