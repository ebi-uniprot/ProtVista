...
var
    ...
    _mutationTypes = ["disease", "deleterious", "intermediate", "benign", "no_prediction"],
    ...
    _scoreRanges = {"deleterious":[0.6, 2], "intermediate":[0.4,0.6], "benign":[-2,0.4]},
    _typeVariantsList = "TypeVariantsList",
    _predictors = {
        "sift": {
            "scoreAttr": "siftScore",
            "predictionAttr": "siftPrediction",
            "shouldInvertScore": true
        },
        "polyphen": {
            "scoreAttr": "polyphenScore",
            "predictionAttr": "polyphenPrediction",
            "shouldInvertScore": false
        }
    }
;
 /* ----------
...
var
    ...
    _variantParser = function(that, categoryViewer){
        ...
        _.each(categoryViewer.opt.category.variants, function(variant, variantArrayPosition) {
            ...
            var scoreAndPredictors = _calculateAverageScore(variant);
            variant.averageScore = scoreAndPredictors.averageScore;
            variant.averagePrediction = _calculateMixedPrediction(that, variant, scoreAndPredictors, position, true);
            ...
            var predictionKey = variant.averagePrediction + _typeVariantsList;
    ...
    /**
     * Calculates the average score based on all predictors declared in the private variable _predictors;
     * it also finds the number of valid predictor (the ones actually used within the variant).
     * @param variant
     * @private
     */
    _calculateAverageScore = function(variant) {
        var averageScore = 0;
        var score = 0;
        var validPredictors = 0;
        _.each(_predictors, function(predictor) {
            if (variant[predictor.predictionAttr] !== "-") {
                score = +variant[predictor.scoreAttr];
                if (predictor.shouldInvertScore === true){
                    score = 1-score;
                }
                averageScore += score;
                validPredictors++;
        ...
        return {averageScore: averageScore, validPredictors: validPredictors};
    },
    /**
     * Calculates the final predicted mutation based on the average score, and update the total number of variants
     * of such mutation type in the current position.
     * @private
     */
    _calculateMixedPrediction = function(that, variant, scoreAndPredictors, position, modifyVariantData) {
        var prediction = null;
        if (scoreAndPredictors.validPredictors !== 0) {
            var key;
            for (key in _scoreRanges){
                var range = _scoreRanges[key];
                if (_isBetween(scoreAndPredictors.averageScore, range[0], range[1])){
                    if (modifyVariantData === true) {
        ...
        if ((prediction === _unknownMutationType) && (variant.isSP === true) && (variant.disease === true)) {
            prediction = _diseasePrediction;
        }
        return prediction;
    },
    /**
...
VariantProteinCategoryFTViewer.getTypes = function(categoryViewer) {
    ...
    _.each(categoryViewer._stylingOpt._consequenceTypes, function(consequenceType) {
        ...
        if (FTVUtils.equalsNoCase(consequenceType, "MISSENSE")) {
            _.each(_mutationTypes, function(mutationType) {
                var predictedTypes = _.filter(variantsType, function(variant) {
                    var scoreAndPredictors = _calculateAverageScore(variant);
                    var averagePrediction = _calculateMixedPrediction(this, variant, scoreAndPredictors, null, false);
                    return FTVUtils.equalsNoCase(averagePrediction, mutationType);
                });
                if (predictedTypes.length !== 0) {
                    types.push({category: consequenceType + "-" + mutationType.replace('_', ' '), type: categoryViewer.opt.category.type, variants: predictedTypes});
                }
            });
        } else {
