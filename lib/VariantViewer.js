...
var drawVariants = function(variantViewer, bars, predictionColour, frequency, fv, container) {
    ...
    variantCircle
        ...
        .attr('fill', function(d) {
            // ECO:0000305
            var color = predictionColour((d.siftScore + (1-d.polyphenScore))/2);
            return (_.findWhere(d.evidences, {code: "ECO:0000305"}))
                ? 'crimson'
                : color === '#NaNNaNNaN' ? 'black' : color;
        })
...
var VariantViewer = function(features, container, fv, width) {
    ...
    this.update = function(manual, automatic) {
        variantViewer.showManual = manual === undefined ? true : manual;
        variantViewer.showAutomatic = automatic === undefined ? true : automatic;
