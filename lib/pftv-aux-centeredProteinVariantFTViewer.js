...
/**
 @class pftv-aux-centeredProteinVariantFTViewer
 */
var CenteredProteinVariantFTViewer;
 
module.exports = CenteredProteinVariantFTViewer = function(){};
/**********
 * Private zone
 **********/
/* ----------
 * Private variables.
 * ---------- */
//Only makes sense for natural variations
var
...
var
    ...
    _init = function(that, categoryViewer) {
        ...
        console.log(that._variantDataObject);
            ...
            _.each(categoryViewer.opt.category.variants, function(variant, variantArrayPosition) {
                ...
                if (position !== previousPosition) {//we have a new position, initiate position object with counters in 0
                    ...
                    that._variantDataObject[position] = {
                        ...
                        "total": 0,
                        "totalFrequency": 0.0,
                        "residue": FTVUtils.getResidue(categoryViewer.opt.sequence[position-1]) + " [" + position + ", " + position + "]"
                        //"variantInfo": []
                    };
                ...
                //Link to the original variant in the category features
                //that._variantDataObject[position]["variantInfo"].push(variantArrayPosition);
                //Link to the original variant in the category features depending on the mutation type (we have mutation type lists will links to the actual variants)
                var predictionKey = prediction + _typeVariantsList;
                that._variantDataObject[position][predictionKey].push(variantArrayPosition);
                //Now add to the count count how many of each consequence we have so far and add any new to the general array containing all of them
                var consequenceTypes = variant.consequenceTypes.replace(" ", "_").split(",");
                consequenceTypes.map(function(consequenceType){
                    ...
                    if(!that._variantDataObject[position].hasOwnProperty(consequenceType)){
                        that._variantDataObject[position][consequenceType] = 0;
                    }
                    that._variantDataObject[position][consequenceType] += 1;
                });
            });
            //that._maximumZoomedHeight = categoryViewer._height - categoryViewer._rulerMargin.top - 25; //TODO
            //that._oldRange = that._maximumVarFrequency - that._minimumVarFrequency; //TODO
            //that._newRange = that._maximumZoomedHeight - that._minimumZoomHeight; //TODO
            //that._availablePlottingHeight = categoryViewer._height - categoryViewer._rulerMargin.top; //TODO
            //that._scaleFactor = Math.floor(categoryViewer._height / that._maximumTotalVar); //TODO that._availablePlottingHeight / that._maximumTotalVar
            that._yScale = d3.scale.linear().
                domain([that._minimumVarFrequency, that._maximumVarFrequency]).
                range([FTVUtils.getMaxShapeSize(), Math.floor(categoryViewer._height / that._maximumTotalVar)]);//TODO should it be 10 here rather than 13 maxShapeSize?
            /*return {
                "minimumFrequency": that._minimumVarFrequency,
                "maximumFrequency": that._maximumVarFrequency,
                "availablePlottingHeight": categoryViewer._height, //that._availablePlottingHeight
                "oldRange": that._oldRange,
                "newRange": that._newRange,
                "overviewScaleFactor": that._scaleFactor
            };*/
        }
    },
    ...
    /** TODO do we really need it? is re-calculating really that time-consuming?
     * Initializes the zoomed SVG as it will be hidden/shown rather than re-calculated.
     * @param that
     * @param categoryViewer
     * @private
     */
    _initZoomedSVG = function(that, categoryViewer) {
        var tempSVGWidthWithMargin = categoryViewer.opt.width - FTVUtils.getTitlesWidth();
        categoryViewer._zoomedFeatureSVG = categoryViewer._featuresDiv
            .append("svg")
            .attr("index", function () {
                return categoryViewer.opt.myArrayIndex;
            })
            .attr("width", tempSVGWidthWithMargin)
            .attr("height", categoryViewer._height)
            .style("display", "none");
    },
    /**
     *
    ...
    _calculatePosition = function (that, categoryViewer, feature, shift) {
        ...
        feature.width = scaledEnd - scaledStart + categoryViewer._pixelPerAA;
    ...
    _getFeatureTooltipText = function(positionSummary, feature) {
        ...
        text = text + "Residue: " + positionSummary.residue + " --> " + mutation + "<br/>";
        //prediction
 ...
 * Public variables.
 * ---------- */
CenteredProteinVariantFTViewer.prototype._scaleFactor = 5;
CenteredProteinVariantFTViewer.prototype._activeMutationTypes = undefined;
CenteredProteinVariantFTViewer.prototype._activeScoreKeys = undefined;
CenteredProteinVariantFTViewer.prototype._activeScoreValues = undefined;
CenteredProteinVariantFTViewer.prototype._zoomedFeatureSVG = undefined;
CenteredProteinVariantFTViewer.prototype._variantDataObject = undefined;
CenteredProteinVariantFTViewer.prototype._consequenceTypesList = undefined;
CenteredProteinVariantFTViewer.prototype._maximumTotalVar = 0;
CenteredProteinVariantFTViewer.prototype._maximumTotalVarFrequency = 0.0;
CenteredProteinVariantFTViewer.prototype._maximumVarFrequency = 0.0;
CenteredProteinVariantFTViewer.prototype._minimumVarFrequency = 0.0;
 ...
 * */
CenteredProteinVariantFTViewer.paintFeatures = function(categoryViewer) {
    var self = this;
    ...
    _.each(self._variantDataObject, function(positionObject) {
        ...
        _.each(self._activeMutationTypes, function(mutationType) {
            ...
            var shift = 0;
            categoryViewer._featureSVGGroup.selectAll("#" + categoryViewer.opt.target + " path." + typeClass + " path." + mutationClass + " path." + positionClass)
                .data(function() {
                ...
                .attr("index", function (varArrayPos, position) {
                    var feature = categoryViewer.opt.category.variants[varArrayPos];
                ...
                .attr("d", function (varArrayPos) {
                    ...
                    shift = feature.height;
                    return FTVUtils.getShape("POSITION", feature, categoryViewer._pixelPerAA, categoryViewer.opt.useShapes, typeClass);
                })
            ;
        });
    });
