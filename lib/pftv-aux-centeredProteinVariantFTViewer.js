...
var
    ...
    _adjustHeight = function(that, categoryViewer) {
        ...
        var heightMaxPossibleVarMaxFreq = Math.floor(categoryViewer.opt.ftHeight / that._maximumTotalVar);
        var heightMaxVarMinHeight = FTVUtils.getMaxShapeSize() * that._maximumTotalVar;
        var heightMaxVarWithMaxFreq = Math.ceil(1 / that._maximumVarFrequency) * heightMaxPossibleVarMaxFreq;
        var heightMaxVarWithMaxRealFreq = (heightMaxPossibleVarMaxFreq * that._maximumTotalVar) * (1/that._maximumTotalVar) / that._maximumVarFrequency;
        //adjust the divs an svg height
        categoryViewer.opt.ftHeight = Math.max(heightMaxVarMinHeight, heightMaxVarWithMaxFreq, heightMaxVarWithMaxRealFreq);
    ...
    _createYScale = function(that, categoryViewer) {
        ...
        console.log("ftHeight " + categoryViewer.opt.ftHeight);
        console.log("minFreq " + that._minimumVarFrequency);
        console.log("maxFreq " + that._maximumVarFrequency);
        console.log("maxVar " + that._maximumTotalVar);
        console.log("scale minF " + that._yScale(that._minimumVarFrequency));
        console.log("scale maxF " + that._yScale(that._maximumVarFrequency));
        console.log("max shape" + FTVUtils.getMaxShapeSize());
        console.log("floor " + Math.floor(categoryViewer.opt.ftHeight / that._maximumTotalVar));
    },
        ...
        _.each(self._activeMutationTypes, function(mutationType) {
            ...
            categoryViewer._featureSVGGroup.selectAll("#" + categoryViewer.opt.target + " path." + typeClass + " path." + mutationClass + " path." + positionClass)
                ...
                .attr("index", function (varArrayPos) {
                    ...
                    feature.featureIndex = varArrayPos;
                    return categoryViewer.opt.myArrayIndex;
                })
                ...
                .on("mouseover", function (varArrayPos) {
                    var feature = categoryViewer.opt.category.variants[varArrayPos];
                    categoryViewer._onMouseOverFeature(categoryViewer, this, feature);
                })
                ...
                .on("mouseout", function (varArrayPos) {
                    var feature = categoryViewer.opt.category.variants[varArrayPos];
                    categoryViewer._onMouseOutFeature(categoryViewer, this, feature);
                })
                .on("click", function(varArrayPos) {
                    var feature = categoryViewer.opt.category.variants[varArrayPos];
                    categoryViewer._onClickFeature(categoryViewer, this, feature);
                })
            ;
        });
    });
