...
var
    ...
    _arrowDown = "\u25BC",
    _defaultOptions = {
        target: "YourOwnDivId" //container id
        ,width: 600
        ,darkBackground: true
        ,useShapes: true
        ,useTooltips: false
...
var
    ...
    _initSequenceLine = function(self) {
        if (self.opt.useShapes) {
            if (self.categoryContains(FTVUtils.getFTLocation().position)) {//we have shapes
                if (self.categoryContains(FTVUtils.getFTLocation().continuous))  {//also rectangles
                    if (self.categoryContains(FTVUtils.getFTLocation().bridge)) {//also bridges
                    ...
                    if (self.categoryContains(FTVUtils.getFTLocation().bridge)) {//but bridges
                        self._svgBottomGap = -1;
            ...
            } else if (self.categoryContains(FTVUtils.getFTLocation().bridge)) {//no shapes but bridges (and possible rectangles)
            ...
            if (self.categoryContains(FTVUtils.getFTLocation().bridge)) {//bridges (and possibly rectangles)
                self._sequenceLine = self.opt.ftHeight + self._gapToBridge + 4; //+ _margin.top;
                self._svgBottomGap = -1;
            } else { //only rectangles
    ...
    _paintFeatures = function(self) {
        ...
        //Add interaction
        self._featureSVGGroup.selectAll("#" + self.opt.target + " path")
            .on("mouseover", function (feature) {
                _onMouseOverFeature(self, this, feature);
            })
            ...
            .on("mouseout", function (feature) {
                _onMouseOutFeature(self, this, feature);
            })
            .on("click", function(feature) {
                _onClickFeature(self, this, feature);
            })
        ;
    },
    /**
     ...
     * @private
     */
    _onClickFeature = function (self, caller, feature) {
     ...
     * @private
     */
    _onMouseOutFeature = function (self, caller, feature) {
     ...
     * @private
     */
    _onMouseOverFeature = function (self, caller, feature) {
        var elem = d3.select(caller);
 ...
 * @private
 * */
ProteinCategoryFTViewer.prototype.categoryContains = function categoryContains(ftClass) {
...
ProteinCategoryFTViewer.prototype.zoomInOut = function zoomInOut(moveToAA) {
    if (this._zoomed === false) {//it is zoomed out, then zoom it in, i.e, go to detailed view
        ...
        this._zoomed = true;
        this.trigger('zoomIn', {category: this.opt.category});
        ...
        this._featureSVG.attr("width", this._svgWidthWithMargin);
        this._zoomed = false;
        this.trigger('zoomOut', {category: this.opt.category});
    }
    _paintFeatures(this);
