    ...
    return {
        createSVG: function(container, width, height, fv, clazz) {
            var svg = container
                ...
                .attr('height', height)
                .on('click', function() {
                    if (!fv.overFeature && fv.selectedFeature ) {
        ...
        .append('title')
            .text( function(d){
                ...
                if (d.alternativeSequence) {
                    var originalSeq = fv.sequence.substring(+d.begin - 1, +d.begin - 1 + d.alternativeSequence.length);
                    text += ', \nConflict: ' +  originalSeq + ' > ' + d.alternativeSequence;
                }
                if (d.mutation) {
