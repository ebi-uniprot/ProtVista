...
var Tooltip = function(d, sequence, container) {
    ...
    var getEvidenceText = function(code, sources) {
        ...
        if ((acronym === 'EXP') || (acronym === 'NAS')) {
            ...
            return sources.length === 0 ? 'Sequence Analysis': sources[0].name + ' annotation';
        } else if ((acronym === 'MIXM') || (acronym === 'MIXA')) {
            return 'Combined sources';
        } else if ((acronym === 'MI') || (acronym === 'AI')){
            return 'Imported';
        } else if (acronym === 'AA') {
            var unirule = _.find(sources, function(source) {
                return source.url && (sources.url.indexOf('unirule') != -1);
            });
            var saas = _.find(sources, function(source) {
                return source.url && (sources.url.indexOf('SAAS') != -1);
            });
            return unirule
                ? 'UniRule annotation'
                : saas ? 'SAAS annotation' : sources[0].name + ' annotation';
        } else {
            return code;
        }
    };
    ...
    tooltip.addEvidences = function(evidences) {
        ...
        _.each(evidences, function(e){
            ...
            typeRow.append('td')
                    .text('Evidence');
            typeRow.append('td')
                .text(getEvidenceText(e.code, e.sources));
