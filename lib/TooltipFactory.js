...
var Tooltip = function(fv, catTitle, d, container) {
    ...
    if (fv.pinPad) {
        ...
        pinContainer.append('div').attr('class', 'up-pp-icon-pin up_pp_icon up_pp_clickable-icon')
            .on('click', function() {
                if (tooltip.data.pinned === true) {
                    ...
                    //fv.pinPad.addElement(tooltip.pinData); //TODO: remove pin
        ...
        th.append('div').style('display', 'inline-block')
            .text(tooltipTitle);
    } else {
        descRow.append('th')
            .attr('colspan',2)
...
var VariantTooltipViewer = function(tooltip) {
    var addPredictions = function(section) {
    ...
    var addAssociation = function(section) {
        if (tooltip.data.association) {
            ...
            _.each(tooltip.data.association, function(association){
                if (association.name) {
                    ...
                    section.information.subsection_disease.information.disease = association.name;
                }
                ...
                if (association.moreInfo) {
                    ...
                    _.each(groupedSources, function(elem, index) {
                        ...
                        moreInfo.append('td').text('See ' + index);
                        var list = moreInfo.append('td');
                        var listText = [];
                        _.each(elem, function(el, i) {
                            list.append('span').append('a').attr('href', el.url)
                                .attr('target', '_blank').text(el.id);
                            listText.push({value: el.id, link: el.url});
                        });
                        section.information.subsection_disease.information['See_' + index] = listText;
                    });
                }
            });
        }
    ...
    var addMutation = function(section) {
    ...
    if (tooltip.data.sp && Evidence.isLSS(tooltip.data.evidences)) {
        ...
        addMutation(tooltip.pinData.sections[0]);
        ...
        addAssociation(tooltip.pinData.sections[length-1]);
        ...
        addPredictions(tooltip.pinData.sections[length-1]);
        tooltip.addEvidences(lssEvidences, tooltip.pinData.sections[length-1]);
    } else {
        addMutation(tooltip.pinData.sections[0]);
        addPredictions(tooltip.pinData.sections[0]);
        tooltip.addEvidences(tooltip.data.evidences, tooltip.pinData.sections[0]);
        addAssociation(tooltip.pinData.sections[0]);
    }
};
...
var TooltipFactory = function() {
    return {
        createTooltip: function(fv, catTitle, data, container) {
            ...
            console.log(JSON.stringify(tooltip.pinData));
            return tooltip;
        }
    };
}();
