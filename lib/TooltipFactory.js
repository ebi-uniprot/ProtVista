...
var addEvidenceLinks = function(tooltip, section, info) {
    ...
    section.information['styled_evidenceArray_' + info.index + '_' + info.counter] = {
        key: info.index,
        key_right: true,
        value: []
    };
    ...
    _.each(info.elem, function(el, i) {
        ...
        section.information['styled_evidenceArray_' + info.index + '_' + info.counter].value.push({
            value: el.id,
            link: url
        });
    });
...
Tooltip.prototype.addEvidences = function(evidences, section) {
    ...
    _.each(evidences, function(e, counter) {
        ...
        _.each(groupedSources, function(elem, index) {
            addEvidenceLinks(tooltip, section, {
                counter: counter, elem: elem, index: index
            });
            if (index === 'PubMed') {
                addEvidenceLinks(tooltip, section, {
                    counter:counter, elem: elem, index: 'EuropePMC', alternative: true
                });
            }
        });
    });
