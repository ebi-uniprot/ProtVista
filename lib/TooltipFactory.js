...
var parseVariantDescription = function(data) {
    if (data.description) {
        ...
        if (index !== -1) {
            ...
            var endIndex = data.description.indexOf('; ', index);
            if (endIndex === (index + 16)) {
                data.description = (data.description.slice(0, index) + data.description.slice(index+18)).trim();
            } else {
        ...
        if (index !== -1) {
            var endIndex = data.description.indexOf('; ', index);
            if (endIndex !== -1) {
                data.lss_description = data.description.slice(index+5, endIndex).trim();
                data.up_description = data.description.slice(endIndex+2).trim();
            } else {
