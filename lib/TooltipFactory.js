...
var addFtId = function(tooltip) {
    if (tooltip.data.ftId !== undefined) {
        ...
        tooltip.pinData.sections[0].information.styled_feature_id = {
            key: "Feature ID",
...
var addDescription = function(tooltip, description){
    if (description) {
        ...
        dataDes.append('td').text(description);
        tooltip.pinData.sections[0].information.description = description;
    }
};
...
var Tooltip = function(fv, catTitle, d, container) {
    ...
    if (tooltip.data.sp !== undefined) {
        ...
        if (sourceText === 'UniProt') {
            addFtId(tooltip);
            addDescription(tooltip, tooltip.data.up_description);
        } else if (sourceText === 'Large scale studies'){
            addDescription(tooltip, tooltip.data.lss_description);
        }
    } else {
        addFtId(tooltip);
        addDescription(tooltip, tooltip.data.description);
    }
};
