...
var
    /**
     * Constructor, it loads the default values for the options, registeres the new instance,
     * and clones objects that should be kept at instance level such as biojs_vis_proteinFeaturesViewer.opt and _ftPosAndHeight.
    ...
    constructor = function(options) {
        ...
        if (allEmpty === true) {
            d3.select("#" + options.target).text("No features available for this protein");
             ...
             _instanceArray[ _instanceIndex].opt = jQuery.extend(true, {}, biojs_vis_proteinFeaturesViewer.opt);
            //debug
            //console.log( _instanceArray);
             _init();
        }
    },
    ...
    _init = function() {
        ...
        var sequenceScale = d3.scale.linear()
            ...
            .range([self._rulerMargin.left, (biojs_vis_proteinFeaturesViewer.opt.width - FTVUtils.getTitlesWidth()) - self._rulerMargin.right]);
         _pixelPerAA = sequenceScale(2) - sequenceScale(1);
        if ( _pixelPerAA > FTVUtils.getMaxPixelAA()) {
            //var tempWidth = biojs_vis_proteinFeaturesViewer.opt.featuresModel.sequence.length * FTVUtils.getMaxPixelAA() +  _rulerMargin.right +  _rulerMargin.left;
            biojs_vis_proteinFeaturesViewer.opt.width = biojs_vis_proteinFeaturesViewer.opt.featuresModel.sequence.length * FTVUtils.getMaxPixelAA() + FTVUtils.getTitlesWidth() + self._rulerMargin.left + self._rulerMargin.right;
            sequenceScale = d3.scale.linear().
                domain([1, biojs_vis_proteinFeaturesViewer.opt.featuresModel.sequence.length + 1])
                //.range([self._rulerMargin.left, tempWidth - self._rulerMargin.right]);
                .range([self._rulerMargin.left, (biojs_vis_proteinFeaturesViewer.opt.width - FTVUtils.getTitlesWidth()) - self._rulerMargin.right]);
             _pixelPerAA = sequenceScale(2) - sequenceScale(1);
            console.log("MAX: " +  _pixelPerAA);
            //biojs_vis_proteinFeaturesViewer.opt.width = tempWidth;
        }
    ...
    _initCategories = function() {
        ...
        _.each(self.opt.featuresModel.categories, function(category, posCat) {
            ...
            myInstance.opt.category[self.opt.target + "_category_" + posCat + self._hasBeenOpenedProp] = false;
            myInstance.on("categoryOpen", function(obj) {
                self._openCategory(obj.category, obj.target, obj.darkBackground);
            });
            myInstance.on("categoryClose", function(obj) {
                self._closeCategory(obj.category, obj.target);
            });
            myInstance.on("featureClicked", function(obj) {
                self._onInstanceFeatureClick(obj);
            });
            myInstance.on("featureOn", function (obj) {
                self._onInstanceFeatureOn(obj);
            });
            myInstance.on("featureOff", function(obj) {
                self._onInstanceFeatureOff(obj);
            });
    ...
    _initControls = function(controlDivUp) {
        ...
        var leftControlDivUpRight = controlDivUpRight.append("div").classed("left", true);
         _zoomInButton = leftControlDivUpRight.append("div")
            .attr("id", "zoomInButton")
            //.attr("src", "../resources/images/zoomIN.png")
            .classed("clickable spriteZoomIn", true)
            .on("click", function() {
                if (self._debug === true) self._debugTrace += ", on-click-zoomInButton";
                self.zoomInOut();
            })
        ;
         _zoomOutButton = leftControlDivUpRight.append("div")
            .attr("id", "zoomOutButton")
            //.attr("src", "../resources/images/zoomOUT.png")
            .classed("clickable spriteZoomOut", true)
            .on("click", function() {
            ...
            .style("display", "none")
        ;
         _moveLeftButton = leftControlDivUpRight.append("div")
            .attr("id", "moveLeftButton")
            //.attr("src", "../resources/images/previousPAGE.png")
            .classed("clickable spriteMoveLeft", true)
            .style("display", "none")
            .on("click", function() {
                if (self._zoomed === true) {
                    if (self._debug === true) self._debugTrace += ", move-left";
                    self.translate(self._slide(true));
                }
            })
            .on("mouseover", function() {
                FTVUtils.mouseover(self._tooltipdiv, "Move current display 50% left");
            })
        ...
        ;
         _moveRightButton = leftControlDivUpRight.append("div")
            .attr("id", "moveRightButton")
            //.attr("src", "../resources/images/nextPAGE.png")
            .classed("clickable spriteMoveRight", true)
            .style("display", "none")
            .on("click", function() {
                if (self._zoomed === true) {
                    if (self._debug === true) self._debugTrace += ", move-right";
                    self.translate(self._slide(false));
                }
            })
            .on("mouseover", function() {
                var elem = d3.select(this);
                elem.style("fill-opacity", 1);
                FTVUtils.mouseover(self._tooltipdiv, "Move current display 50% right");
            })
            .on("mousemove", function() {
                FTVUtils.mousemove(self._tooltipdiv);
            })
        ...
        _openAllButton = rightControlDivUpRight.append("div")
            .attr("id", "openAllButton")
            //.attr("src", "../resources/images/open.png")
            .classed("clickable spriteOpenAll", true)
        ...
        _closeAllButton = rightControlDivUpRight.append("div")
            .attr("id", "closeAllButton")
            //.attr("src", "../resources/images/close.png")
            .classed("clickable spriteCloseAll", true)
            .style("display",  _buttonDisplay)
            .on("click", function() {
                if (self._debug === true) {
        ...
        _settingsButton = rightControlDivUpRight.append("div")
            //.attr("src", "../resources/images/settings.png")
        ...
        _helpButton = rightControlDivUpRight.append("div")
            .attr("id", "helpButton")
            //.attr("src", "../resources/images/help.png")
            .classed("clickable spriteHelp", true) //TODO onclick
            .style("display",  _buttonDisplay)
            .on("click", function() {
                if (self._debug === true) self._debugTrace += ", STOP";
    ...
    _initDragger = function(rulerTickHeight) {
        var self = this;
         _dragger =  _ruler.append("g")
            ...
            .attr("d", function(d) {
                return self._draggerPath(d, self._rulerWidth, rulerTickHeight+self._draggerGap);
            })
            .attr("index", self._instanceIndex)
            .on("click", function() {
...
biojs_vis_proteinFeaturesViewer.translate = function(moveToAA) {
    ...
    if ( _zoomed === true) {
        ...
        var firstMaxVisibleAA =  _rulerWidth -  _dragger.attr("width");// + self._pixelPerAA;
        if (currentX < 0) {
            currentX = 0;
        } else if (currentX > firstMaxVisibleAA) {
            currentX = firstMaxVisibleAA;
