...
var
    ...
    _initCategories = function(self) {
        ...
        var container = d3.select('#' + self.opt.target)
                           .append('div')
                                .attr('class','fv-category-container');

        var xScale = d3.scale.linear()
                                .domain([1,  features.sequence.length + 1])
                                .range([0, self.opt.width]);

        addCategory(CategoryFactory.createCategory(features.domainsAndSites,'basic', container, xScale));
        addCategory(CategoryFactory.createCategory(features.moleculeProcessing,'basic', container, xScale));
        addCategory(CategoryFactory.createCategory(features.mutagenesis,'basic', container, xScale));
        addCategory(CategoryFactory.createCategory(features.ptm,'basic', container, xScale));
        addCategory(CategoryFactory.createCategory(features.seqInfo,'basic', container, xScale));
        addCategory(CategoryFactory.createCategory(features.structural,'basic', container, xScale));
        addCategory(CategoryFactory.createCategory(features.topology,'basic', container, xScale));
        addCategory(CategoryFactory.createCategory(features.variants,'variant', container, xScale));
