...
var
    ...
    _constructor = function(self, options) {
        var targetID = "up_pftv_" + (new Date().getTime());
    ...
    _initTooltip = function(self) {
        ...
        self._helpDiv = d3.select("#" + self.opt.target).append("div")
            ...
            .style("opacity", 1e-6)
            .style("z-index", -1);
        //<tspan id="corrtitle" x="400" y="659.3333333333334" dominant-baseline="middle" text-anchor="middle">Change the parameter ?topic=&lt;any number between 100 and 149 except 135&gt; to see other topic.</tspan>
        self._helpDiv.append("div").classed("clickable spriteCloseX noBodyClick", true)
            .on("click", function() {
                self._helpDiv
                    .style("opacity", 1e-6)
                    .style("z-index", -1);
        ...
        ;
        self._helpDiv.append("div").classed("noBodyClick", true)
            .html("<p class='noBodyClick'>The Protein Features Viewer displays protein sequence features in tracks. " +
            ...
            "<div class='noBodyClick'>" + _ARROW_RIGHT + " opens a category so its types will be displayed.</div>" +
            "<div class='noBodyClick'>" + _ARROW_DOWN + " closes a category.</div>" +
            "<div>" +
                "<div class='noBodyClick' style='display: inline-block'>" +
                "   <div class='spriteOpenAll noBodyClick' style='display:inline-block;'></div> Opens all categories. " +
                "</div>" +
                "<div class='noBodyClick' style='display: inline-block'>" +
                "   <div class='spriteCloseAll noBodyClick' style='display:inline-block;'></div> Closes all categories" +
                "</div>" +
            "</div>" +
            "<div class='noBodyClick'>" +
                "<div class='spriteSettings noBodyClick' style='display:inline-block;'></div> Allows you to hide/show categories." +
            "</div>" +
            "<div>" +
                "<div class='noBodyClick' style='display: inline-block'>" +
                "   <div class='spriteZoomIn noBodyClick' style='display:inline-block;'></div> Zooms in to amino acid level. " +
                "</div>" +
                "<div class='noBodyClick' style='display: inline-block'>" +
                "   <div class='spriteZoomOut noBodyClick' style='display:inline-block;'></div> Zooms out." +
                "</div>" +
            "</div>" +
            "<div>" +
                "<div class='noBodyClick' style='display: inline-block'>" +
                "   <div class='spriteMoveLeft noBodyClick' style='display:inline-block;'></div> Moves to the left. " +
                "</div>" +
                "<div class='noBodyClick' style='display: inline-block'>" +
                "   <div class='spriteMoveRight noBodyClick' style='display:inline-block;'></div> Moves to the right." +
                "</div>" +
            "</div>");
        /*openText.append("span ").text("Use " + _ARROW_RIGHT + " to open a category so its types will be displayed. " +
            "Use " + _ARROW_DOWN + " to close it. " +
            "You can also open all categories ");
        openText.append("span").classed("spriteOpenAll", true);
        openText.append("span").text(", or close them all ");
        openText.append("span").text("spriteCloseAll");*/
    },
    /**
    ...
    _initControls = function(self, controlDivUp) {
        ...
        self._helpButton = rightControlDivUpRight.append("div")
            .attr("id", "helpButton")
            .classed("clickable spriteHelp noBodyClick", true)
            .style("display",  _BUTTON_DISPLAY)
            ...
            internalDiv.append("input")
                ...
                .on("click", function() {
                    if (this.checked === true) {
                        ...
                        d3.selectAll("#" + self.opt.target + "_category_" + d3.select(this).attr("index") + " div.hiddenCategory")
                            .style("display", "none"); //hide category if closed
                        d3.selectAll("div[id^='" + self.opt.target + "_category_" + d3.select(this).attr("index") + "_type_']" + ".hiddenCategory")
