...
var Category = function(data, container, xScale, width, zoom) {
	...
	category.addTrack = function(track) {
        ...
        var that = this;
        track.on('selectedFeature', function(obj) {
            that.trigger('selectedFeature', obj);
        });
	};
	...
	category.buildTracks = function() {
		...
		_.each(_.keys(typeFeatures), function(d) {
			...
			that.addTrack(TrackFactory.createTrack(typeFeatures[d], 'basic', category.tracksContainer, category.xScale, zoom));
		});
	};
	...
	category.propagateSelection = function() {		
		if (Global.selectedFeature) {
			d3.selectAll('svg path[name=' + Global.selectedFeature.internalId + ']')
	...
	category.selectFeature = function(feature, elem) {
		...
		this.trigger('selectedFeature', {"feature": Global.selectedFeature, "clazz": clazz});
	};
};
			...
			selection.each(function(data) {
				...
				shapes
					...
					.classed('up_pftv_activeFeature', function(d) {					
						return d === Global.selectedFeature;
					})
					.on('click', function(d){
						tooltipHandler(d);
