...
var VariantCategoryViewer = function(category) {
    ...
    var features = category.data.features,
        allFeatures = category.data.features,
    ...
    var updateVariationCountArray = function(features) {
        return _.map(features, function(d) {
            return d.variants.length;
        });
    };

    var variationCountArray = updateVariationCountArray(features);
    ...
    this.update = function(manual, automatic) {
        if ((manual !== undefined) || (automatic !== undefined)) {
            if ((manual) && (automatic)) {
                ...
                features = ViewerFactory.getManualFeatures(category.fv.evidenceMapping, allFeatures);
            } else if ((manual !== true) && (automatic)) {
                features = ViewerFactory.getAutomaticFeatures(category.fv.evidenceMapping, allFeatures);
            } else {
                features = [];
            }

            variationCountArray = updateVariationCountArray(features);
