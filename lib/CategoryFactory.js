...
var Category = function(data, container, xScale, width, zoom, options) {
	var category = this;
	category.tracks = [];
	category.data = data;
	...
	category.buildTracks = function() {
		...
		_.each(_.keys(typeFeatures), function(d) {
			...
			addTrack(TrackFactory.createTrack(typeFeatures[d], 'basic', category.tracksContainer, category.xScale, zoom, category.options));
		});
	};
...
Category.basic = function() {
	this.categoryViewer = new BasicCategoryViewer(this.data.features, this.viewerContainer, this.xScale, this.width, this.zoom, this.options);
};

Category.variant = function() {
	this.categoryViewer = new VariantCategoryViewer(this.data.features, this.viewerContainer, this.xScale, this.width, this.zoom, this.options);
};
...
//Viewer types
var BasicCategoryViewer = function(features, container, xScale, width, zoom, options) {
			...
			selection.each(function(data) {
				...
				shapes
					...
					.on('click', function(d,i){
						tooltipHandler(d);
						Global.selectedFeature = (d === Global.selectedFeature) ? undefined : d;
						var selectedPath = d3.select(this).classed('up_pftv_activeFeature');
						d3.selectAll('svg path.up_pftv_activeFeature').classed('up_pftv_activeFeature', false);
						d3.select(this).classed('up_pftv_activeFeature', !selectedPath);			
					});
...
var VariantCategoryViewer = function(features, containerId, xScale, width, zoom, options) {
	this.update = function() {
		//TODO
	}
	return this;
...
var CategoryFactory = function() {
	return {
		createCategory: function(data, type, container, xScale, width, zoom, options) {
			...
			Category[type].prototype = new Category(data, container, xScale, width, zoom, options);
