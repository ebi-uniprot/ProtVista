...
var Category = function(data, container, xScale, width, zoom, selectedFeature) {
	var category = this;
	category.tracks = [];
	category.data = data;
	...
	category.zoom = zoom;
	category.selectedFeature = selectedFeature;
	...
	category.buildTracks = function() {
		...
		_.each(_.keys(typeFeatures), function(d) {
			...
			addTrack(TrackFactory.createTrack(typeFeatures[d], 'basic', category.tracksContainer, category.xScale, zoom, selectedFeature));
		});
	};
...
Category.basic = function() {
	this.categoryViewer = new BasicCategoryViewer(this.data.features, this.viewerContainer, this.xScale, this.width, this.zoom, this.selectedFeature);
};

Category.variant = function() {
	this.categoryViewer = new VariantCategoryViewer(this.data.features, this.viewerContainer, this.xScale, this.width, this.zoom, this.selectedFeature);
};
...
//Viewer types
var BasicCategoryViewer = function(features, container, xScale, width, zoom, selectedFeature) {
			...
			selection.each(function(data) {
				...
				shapes
					...
					.classed('up_pftv_activeFeature', function(d) {						
						return (d.selected) ? d.selected : false;
					})
					.on('click', function(d,i){
						tooltipHandler(d);
						d.selected = (d.selected) ? !d.selected : true;
						var selected = d3.select(this).classed('up_pftv_activeFeature');
						d3.selectAll('.up_pftv_activeFeature').classed('up_pftv_activeFeature', false);	
						d3.select(this).classed('up_pftv_activeFeature', !selected);				
					});
...
var VariantCategoryViewer = function(features, containerId, xScale, width, zoom, selectedFeature) {
	this.update = function() {
		//TODO
	}
	return this;
...
var CategoryFactory = function() {
	return {
		createCategory: function(data, type, container, xScale, width, zoom, selectedFeature) {
			...
			Category[type].prototype = new Category(data, container, xScale, width, zoom, selectedFeature);
