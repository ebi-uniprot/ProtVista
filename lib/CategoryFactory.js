...
var Category = function(data, type, fv) {
    ...
    category.header = categoryContainer.append('a')
        ...
        .text(fv.shortenDisplayName(data.label))
        .on('click', function() {
            category.toggle();
...
Category.prototype.update = function(manual, automatic) {
    var category = this;
    category.categoryViewer.update(manual, automatic);
    _.each(category.tracks, function(t) {
        t.update(manual, automatic);
    });
};
...
var BasicCategoryViewer = function(category) {
    return new BasicViewer(
        category.data.features, category.viewerContainer, category.fv, category.fv.width
    );
};
...
var VariantCategoryViewer = function(category) {
    ...
    var updateVariationCountArray = function(features, manual, automatic) {
        return _.map(features, function(d) {
            return _.filter(d.variants, function(ft) {
                return (manual && ft.isManual) || (automatic && ft.isAutomatic) ? true : false;
            }).length;
        });
    };

    var variationCountArray = updateVariationCountArray(features, true, true);
    var manualVariationCountArray = updateVariationCountArray(features, true, false);
    var automaticVariationCountArray = updateVariationCountArray(features, false, true);
    var noneVariantCountArray = updateVariationCountArray([], false, false);
    ...
    this.update = function(manual, automatic) {
        ...
        var countArray = manual && automatic ? variationCountArray
            : manual && !automatic ? manualVariationCountArray
            : !manual && automatic ? automaticVariationCountArray
            : noneVariantCountArray;
        varChart.selectAll(".up_pftv_block-area")
            .data(features)
            .attr("class","up_pftv_block-area")
            .attr("d",line(countArray));
