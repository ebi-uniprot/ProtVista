...
var FeatureFactory = function() {
	return {
		getFeature: function(type, aaWidth, height, length, withShapes) {
			var feature,
...
Feature.bridge = function(width, height, length) {
    if (length !== 1) {
    	return 'M' + -(gapRegion) + ',' + height
	            ...
	            + 'L' + (width-gapRegion) + ',2'
	            + 'L' + -(gapRegion) + ',2Z';
	            //S for bridges, could be C but not implemented yet
	            // + 'M ' + (width/2) + ',0' 
				// + 'q ' + -(height+height/2) + ',' + (height/4) + ' ' + -(height/2) + ',' + (height/2)
				// + 'q ' + height + ',' + (height/4) + ' ' + -(height/2) + ',' + (height/2);	
    } else {
    	return 'M' + -(gapRegion) + ',' + height
...
Feature.wave = function(width, height, length) {
	var rx = symbolSize/4;//Math.sqrt(symbolSize / Math.PI);
	var ry = symbolSize/2;
