...
/**
 * Returns a rectangle. 0,0 is in the middle, we first need to move to the starting of the aa.  
...
/**
 * Returns a bridge. 0,0 is in the middle, we first need to move to the starting of the aa.  
 * @param {Object} width Feature width.
 * @param {Object} height Feature height.
 * @param {Object} length Feature length.
 */
Feature.bridge = function(width, height, length) {
    var shape, gap = width/length/2;
    if (length !== 1) {
    	shape = 'M' + -(gap) + ',' + height
	            + 'L' + -(gap) + ',0'
	            + 'L' + (width-gap) + ',0'             
...
Feature.diamond = function(width, height, length) {
	...
	if (length !== 1) {
		shape = shape
			+ 'L0,0'
			+ 'L' + centerx + ',' + centerx
			+ 'L' + (width/length-centerx) + ',' + centerx
			+ 'L' + -(width/length-centerx) + ',' + centerx
			+ 'L' + -centerx + ',' + centerx;
	}	
	return shape + 'Z';
