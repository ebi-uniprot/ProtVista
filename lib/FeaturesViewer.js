...
var FeaturesViewer = function(opts) {
	...
	var init = function(d) {
		...
		fv.viewport = createNavRuler(fvContainer, fv.xScale, d.sequence.length + 1);
		fv.aaViewer = createAAViewer(fvContainer, fv.xScale, d.sequence);
		fv.zoom = createZoom(fv.xScale, d.sequence.length + 1);
	...
	var createAAViewer = function(container, xScale, sequence) {
		...
		var aaSelectorPlot = function(){
			...
			var aaSelectorPlot = function(selection) {
				selection.each(function(data) {
					series = d3.select(this);
					aminoAcids = series.selectAll('.amino_acid_selector')
									.data(data);

					aminoAcids.enter().append('rect');

					aminoAcids.attr('x', function(d, i) {
							gapRegion = (xScale(2)-xScale(1))/2;
							return xScale(d.feature.begin) - gapRegion;
						})
						.attr('y', 0)
						.attr('width', function(d, i) {		
							return ((d.feature.end) ? d.feature.end - d.feature.begin + 1 : 1) * (xScale(2)-xScale(1));
						})
						.attr('height', 10)
						.attr('class', function(d) {
							return 'amino_acid_selector ' + d.clazz;
						});
