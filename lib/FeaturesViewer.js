...
var FeaturesViewer = function(opts) {
	...
	var createAAViewer = function(container, xScale, sequence) {
		...
		//selection group TODO
		var selectorPlot = function(){
			var series,
				selector;

			var selectorPlot = function(selection){
				selection.each(function(data){
					series = d3.select(this);
					selector = series.selectAll('.amino-acid-selector')
									.data(data);
					...
					selector.enter().append('rect')
						...
						.attr('stroke', 'black')
						.attr('x', function() {
							return xScale(1); //TODO gap
						})
						.attr('y', 0)
						.attr('width', 100)
						.attr('height', 10)
						.attr('class','amino-acid-selector');
		...
		var selectorSeries = selectorPlot();
		...
		selectorGroup.datum([0])
			.call(selectorSeries);

		aaViewer.update = function() {
			g.call(series);
			selectorGroup.call(selectorSeries); //TODO pan
