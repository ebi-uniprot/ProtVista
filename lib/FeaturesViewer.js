...
var findFeature = function(fv, ftConsecutiveId) {
    var lookup;
            ...
            lookup =  _.find(datum.features, function(feature) {
                if (feature.variants) {
                    lookup = _.find(feature.variants, function(variant) {
                        return variant.internalId === ftConsecutiveId;
                    });
                    return lookup;
                } else {
                    return feature.internalId === ftConsecutiveId;
                }
            });
            return lookup;
        }
...
FeaturesViewer.prototype.selectFeature = function(ftConsecutiveId) {
    var fv = this;
    var feature = findFeature(fv, ftConsecutiveId);
    var elem = d3.select('[name="' + ftConsecutiveId + '"]');
    if (feature && elem) {
        if (fv.selectedFeature) {
            if (fv.selectedFeature.internalId !== feature.internalId) {
                ViewerFactory.selectFeature(feature, elem.node(), fv);
            }
        } else {
            ViewerFactory.selectFeature(feature, elem.node(), fv);
        }
...
FeaturesViewer.prototype.deselectFeature = function(ftConsecutiveId) {
    var fv = this;
    var feature = findFeature(fv, ftConsecutiveId);
    var elem = d3.select('[name="' + ftConsecutiveId + '"]');
    if (feature && elem && fv.selectedFeature && (fv.selectedFeature.internalId === feature.internalId)) {
        ViewerFactory.selectFeature(feature, elem.node(), fv);
    }
};
