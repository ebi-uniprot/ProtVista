...
var FeaturesViewer = function(opts) {
    ...
    var init = function(d) {
        ...
        if (opts.display) {
            _.each(_.keys(opts.display), function(key) {
                if ((opts.display[key] === true) && (d[key])) {
                    addCategory(CategoryFactory.createCategory(d[key], 'basic', fv));
                }
            });
        } else {
    ...
    fv.load = function() {
        ...
        //TODO Should we use the opts.display here so only selected categories will be retrieved from the ws?
