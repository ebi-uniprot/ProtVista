...
var closeTooltipAndPopup = function(fv) {
    if (!fv.overFeature && !fv.overTooltip) {
        TooltipFactory.removeAllTooltip();
    }
...
var FeaturesViewer = function(opts) {
    var fv = this;
    fv.dispatcher = d3.dispatch("featureSelected", "featureDeselected", "regionHighlighted", "ready",
        "noData", "noFeatures", "error");
    fv.width = 760;
    fv.maxZoomSize = 30;
...
FeaturesViewer.prototype.highlightRegion = function(begin, end) {
    console.log('FeaturesViewer.prototype.highlightRegion');
    console.log(begin);
    console.log(end);
    var fv = this;
    begin = +begin;
    end = +end;
    var goAhead = (begin >= 1) && (begin <= fv.sequence.length);
    goAhead =  goAhead && (end >= 1) && (end <= fv.sequence.length);
    goAhead = goAhead && (begin <= end);
    if (goAhead) {
        fv.highlightedRegion = {begin: begin, end: end};
        console.log(fv.highlightedRegion);
        ViewerHelper.highlightRegion(fv);
    } else {
        fv.dispatcher.error({error: 'Region does not belong to the protein or is not well defined',
            begin: begin, end: end});
    }
};
