...
var FeaturesViewer = function(opts) {
	...
	var init = function(d) {
		...
		addCategory(CategoryFactory.createCategory(d.domainsAndSites, 'basic', container, xScale, width, fv.zoom, d.options));
		addCategory(CategoryFactory.createCategory(d.moleculeProcessing, 'basic', container, xScale, width, fv.zoom, d.options));
		addCategory(CategoryFactory.createCategory(d.ptm, 'basic', container, xScale, width, fv.zoom, d.options));
		addCategory(CategoryFactory.createCategory(d.seqInfo, 'basic', container, xScale, width, fv.zoom, d.options));
		addCategory(CategoryFactory.createCategory(d.structural, 'basic', container, xScale, width, fv.zoom, d.options));
		addCategory(CategoryFactory.createCategory(d.topology, 'basic', container, xScale, width, fv.zoom, d.options));
		addCategory(CategoryFactory.createCategory(d.mutagenesis, 'basic', container, xScale, width, fv.zoom, d.options));
		addCategory(CategoryFactory.createCategory(d.variants, 'variant', container, xScale, width, fv.zoom, d.options));

		createTooltipBox(fvContainer);
	};
	...
	var updateViewportFromChart = function (xScale, maxPos) {
	...
	var createAAViewer = function(container, xScale, sequence) {
		...
		var aaSelectorPlot = function(){
			...
			var aaSelectorPlot = function(selection) {
				selection.each(function(data) {
					series = d3.select(this);
					aminoAcids = series.selectAll('.amino-acid-selector')
									.data(data);

					aminoAcids.enter().append('rect');

					aminoAcids.attr('x', function(d, i) { // TODO coordinates
							return 0;
						})
						.attr('y', aaViewHeight / 2+5) //TODO position
						.attr('width', function(d, i) {		//TODO gap to the left and coordinates			
							return 0;
						})
						.attr('height', 5) //TODO
						.attr('class','amino-acid-selector'); //color and class
		...
		selectorGroup.datum([{"begin": -10, "end": -10}])
			.call(selectorSeries);
		...
		aaViewer.selectFeature = function(feature) {
	...
	var addCategory = function(category) {
		...
		category.on('selectedFeature', function(obj) {
			fv.aaViewer.selectFeature(obj);
		});
	};
