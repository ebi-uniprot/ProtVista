...
var createAAViewer = function(fv, container, sequence) {
    var aaViewer = {}, aaViewWidth = fv.width, aaViewHeight = 15;
    var svg = container
        .append('div')
...
var FeaturesViewer = function(opts) {
    ...
    fv.load = function() {
        Spinner.loadSpinner({width: fv.width, height: 100, container: opts.el, id:'up_pftv_spinner'});
    ...
    fv.applyFilter = function(manual, automatic) {
        if (manual === undefined) {
            manual = true;
        }
        ...
        _.each(filterCategories, function(category) {
            category.update(manual, automatic);
        });
        if (fv.selectedFeature) {
            if ((fv.selectedFeature.isManual !== undefined) && (fv.selectedFeature.isAutomatic !== undefined)) {
                if ( !((fv.selectedFeature.isManual && manual) || (fv.selectedFeature.isAutomatic && automatic))) {
                    ViewerFactory.selectFeature(fv.selectedFeature, fv.selectedFeatureElement, fv);
                }
            }
        }
