...
var findDOMElemById = function(feature) {
    var container = undefined;
    var elem = d3.selectAll('[name="' + feature.internalId + '"]')
        .filter(function(d) {
            var el = d3.select(this);
            if (el.classed('up_pftv_variant') || el.classed('up_pftv_variant_hidden')) {
                var parentDiv = this.parentNode.parentNode.parentNode.parentNode.parentNode;
                container = d3.select(parentDiv);
                return true;
            } else {
                var parentDiv = this.parentNode.parentNode.parentNode;
                var parentEl = d3.select(parentDiv);
                if (parentEl.classed('up_pftv_category-viewer')) {
                    var chosen = parentEl.style('display') !== 'none';
                    if (chosen && (container === undefined)) {
                        container = parentEl;
                    }
                    return chosen;
                } else {
                    var tempContainer = d3.select(parentDiv);
                    parentDiv = parentDiv.parentNode;
                    var chosen = d3.select(parentDiv).style('display') !== 'none';
                    if (chosen && (container === undefined)) {
    ...
    fv.load = function() {
        ...
        dataLoader.done(function(d) {
            ...
            if (d.totalFeatureCount === 0) {
                ...
                fv.dispatcher.noFeatures({data: d});
            } else {
                fv.init(opts, d);
            }
        }).fail(function(e) {
...
FeaturesViewer.prototype.selectFeature = function(ftType, start, end, altSequence) {
    ...
    if (!feature) {
        fv.dispatcher.noFeatures({data: {ftType: ftType, begin: start, end: end}});
    }

    var elAndCont = findDOMElemById(feature);

    if (feature && elAndCont.elem && !elAndCont.elem.classed('up_pftv_variant_hidden')) {
        var catTitle = fv.getCategoryTitle(ftType);
        if (elAndCont.elem.classed('up_pftv_variant')) {
            var varTrack = d3.select('.up_pftv_category-name[title="' + fv.categoryOrderAndType.variants.title + '"]');
            if (varTrack.classed('up_pftv_arrow-right')) {
                var variantsCat = _.find(fv.categories, function(cat) {
                    return cat.data.label === fv.categoryOrderAndType.variants.title;
                });
                if (variantsCat) {
                    variantsCat.toggle();
                }
        ...
        var elemRect = elAndCont.elem.node().getBoundingClientRect();
        var contRect = elAndCont.container.node().getBoundingClientRect();
        var coordinates = {x: elemRect.x - contRect.x, y: elemRect.y - contRect.y};
        if (fv.selectedFeature) {
            if (fv.selectedFeature.internalId !== feature.internalId) {
                ViewerHelper.selectFeature(feature, elAndCont.elem.node(), fv);
            } else {
                fv.dispatcher.featureSelected({feature: fv.selectedFeature, color: elAndCont.elem.style("fill")});
            }
        } else {
            ViewerHelper.selectFeature(feature, elAndCont.elem.node(), fv);
        }
        TooltipFactory.createTooltip(fv, catTitle, feature, elAndCont.container, coordinates);
    }
};
